class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for i in range(len(s2)-len(s1)+1):
            if i>0:
                # windowHashMap[s2[i-1]] = windowHashMap.get(s2[i-1],0) - 1
                windowHashMap[s2[i-1]] = windowHashMap[s2[i-1]] - 1
                windowHashMap[s2[i+len(s1)-1]] = windowHashMap.get(s2[i+len(s1)-1],0) + 1
